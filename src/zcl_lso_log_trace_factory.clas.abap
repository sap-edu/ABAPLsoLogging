  ...
  public section.
    ...
    class-methods create
      importing id               type zlso_log_trace-id
                ...
                http_status      type i
                payloads         type zif_lso_log_payload=>tt_payloads
...
*      rest_client      type ref to if_rest_client
    ...
    class-methods create_trace
      importing trace_parameters type ty_s_custom
      returning value(trace)     type ref to zcl_lso_log_trace .

    class-methods create_by_ids
    ...
    loop at payloads assigning field-symbol(<payload>).
      case <payload>-type.
        when zif_lso_log_payload=>c_type-request.
          insert <payload>-payload into table request_payload.

        when zif_lso_log_payload=>c_type-response.
          insert <payload>-payload into table response_payload.
...
*    trace = zcl_lso_log_trace_factory=>create_trace( trace_param ).
  ...
  method create_by_ids.
    ...
    loop at traces assigning field-symbol(<trace>).
      if not line_exists( objects[ key object_key components id = <trace>-id ] ).
        " Get payload for given trace id.
        data(trace_payload) = value zif_lso_log_payload=>tt_payloads( for p in payloads using key primary_key where ( trace_id = <trace>-id )
                                                                                                                    ( corresponding #( p ) ) ).

        " Get requests headers for given trace id.
        data(request_headers) = value if_web_http_request=>name_value_pairs( for h in headers using key primary_key where ( trace_id = <trace>-id and type = zcl_lso_log_headers=>c_type-request )
                                                                                                                          ( name = h-name value = h-value ) ).

        " Get response headers for given trace id.
        data(response_headers) = value if_web_http_request=>name_value_pairs( for h in headers using key primary_key where ( trace_id = <trace>-id and type = zcl_lso_log_headers=>c_type-response )
                                                                                                                           ( name = h-name value = h-value ) ).

        " Instantiate trace object.
        data(trace) = zcl_lso_log_trace_factory=>create( id               = <trace>-id
        ...
        insert value #( id       = <trace>-id
                        instance = trace ) into table objects.
      endif.
    endloop.
  endmethod.
